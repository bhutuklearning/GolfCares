...
import { mockDraw, mockCharities } from '@/mocks/mockData'
...
export default function HomePage() {
  ...
  const { data: charitiesData } = useQuery({ queryKey: ['featuredCharities'], queryFn: charityApi.getFeatured,  enabled: !useMockData  })

  const draw = useMockData ? mockDraw : drawData?.draw
  const featuredCharities = useMockData ? mockCharities.filter(c => c.isFeatured).slice(0, 3) : charitiesData?.charities || []

  // Generate 20 random numbers for the background floating effect
    ...
    <div className="w-full relative overflow-hidden bg-brand-dark">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center pt-20 pb-12 overflow-hidden">
        ...
        <div className="relative z-10 text-center max-w-4xl mx-auto px-6">
          ...
          >
            <Link to="/register">
              <button className="w-full sm:w-auto px-8 py-4 bg-brand-green text-black font-bold rounded-xl hover:bg-brand-green/90 transition-all shadow-lg shadow-brand-green/25 hover:shadow-brand-green/40 hover:scale-105 active:scale-95 text-lg">
                Subscribe Now →
              </button>
            </Link>
      ...
      <section id="how-it-works" className="py-24 px-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          <div className="text-center">
            ...
            <p className="text-gray-400 mt-4 max-w-xl mx-auto text-lg">Three steps to play golf with purpose</p>
          </div>
          ...
          >
            {[
              ...
              <motion.div
                ...
                whileHover={{ y: -4, borderColor: 'rgba(0,200,150,0.3)' }}
                className="bg-brand-card border border-white/5 rounded-2xl p-8 transition-colors group"
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl ${card.bg}`}>
                  {card.emoji}
                </div>
      ...
      <section className="py-24 px-6 bg-[#0d1326] border-y border-white/5">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-16 max-w-4xl mx-auto">
            {/* 5-Match */}
            <div className="bg-brand-card border border-white/5 rounded-2xl p-8 text-center shadow-lg relative overflow-hidden group hover:border-brand-gold/30 transition-colors">
              ...
              <div className="text-4xl font-black text-brand-gold mt-4 mb-2">
                £{(draw?.prizePool?.fiveMatch || 89.91).toFixed(2)}
              </div>
              <p className="text-sm text-gray-500 font-medium">Jackpot Prize</p>
            </div>

            {/* 4-Match */}
            <div className="bg-brand-card border border-white/5 rounded-2xl p-8 text-center shadow-lg group hover:border-gray-400/30 transition-colors">
              ...
              <div className="text-4xl font-black text-gray-300 mt-4 mb-2">
                £{(draw?.prizePool?.fourMatch || 31.47).toFixed(2)}
              </div>
              <p className="text-sm text-gray-500">Shared Pool</p>
            </div>

            {/* 3-Match */}
            <div className="bg-brand-card border border-white/5 rounded-2xl p-8 text-center shadow-lg group hover:border-brand-accent/30 transition-colors">
              <div className="text-3xl mb-4 group-hover:scale-110 transition-transform">🥉</div>
              <h3 className="text-gray-400 font-bold">3-Number Match</h3>
              <div className="text-4xl font-black text-brand-accent mt-4 mb-2">
                £{(draw?.prizePool?.threeMatch || 22.48).toFixed(2)}
              </div>
              <p className="text-sm text-gray-500">Shared Pool</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-16">
            {featuredCharities.map((charity: any) => (
              <motion.div
                ...
                className="bg-brand-card border border-white/5 rounded-2xl overflow-hidden shadow-lg group hover:border-brand-green/30 transition-all flex flex-col"
              >
                {charity.imageUrl ? (
      ...
      <section className="py-24 px-6 relative overflow-hidden bg-brand-card border-y border-white/5">
        ...
        <motion.div 
          ...
          className="absolute -right-20 -top-40 opacity-[0.03] pointer-events-none"
        >
          <svg width="600" height="600" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <circle cx="50" cy="50" r="28" stroke="currentColor" strokeWidth="2" strokeDasharray="2 6" />
        ...
        <div className="relative z-10 text-center max-w-3xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mt-10">
            ...
            <Link to="/charities">
              <button className="w-full sm:w-auto px-8 py-4 bg-brand-dark border border-white/20 text-white font-medium rounded-xl hover:bg-white/5 transition-all text-lg shadow-lg hover:border-white/40">
                Browse Charities
              </button>
            </Link>
          </div>
